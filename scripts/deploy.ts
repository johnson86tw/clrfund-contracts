...
async function main() {
  ...
  const fundingRoundFactory = await FundingRoundFactory.deploy(maciFactory.address);
  ...
  if (recipientRegistryType === "simple") {
    ...
    recipientRegistry = await OptimisticRecipientRegistry.deploy(UNIT.div(1000), 0, fundingRoundFactory.address);
  } else {
